...
export const addDoctor = async (req, res) => {
  try {
    ...
    if (!plainPassword || !email || !data.name) {
    ...
    let parsedSlots = [];
    try {
      parsedSlots = data.avaliableslots ? JSON.parse(data.avaliableslots) : [];
    } catch (err) {
      parsedSlots = [data.avaliableslots];
    }
